...
root@test3:~# sudo mkdir -p /mnt/data
root@test3:~# sudo nano /etc/fstab
root@test3:~# mc

root@test3:~# sudo mount -o defaults /dev/sdb1 /mnt/data
root@test3:~# mc
...
root@test3:~# systemctl start postgresql
root@test3:~# systemctl status postgresql
● postgresql.service - PostgreSQL RDBMS
     Loaded: loaded (/lib/systemd/system/postgresql.service; enabled; vendor preset: enabled)
     Active: active (exited) since Tue 2022-06-21 05:06:54 UTC; 31min ago
    Process: 716 ExecStart=/bin/true (code=exited, status=0/SUCCESS)
   Main PID: 716 (code=exited, status=0/SUCCESS)

Jun 21 05:06:52 test3 systemd[1]: Starting PostgreSQL RDBMS...
Jun 21 05:06:54 test3 systemd[1]: Finished PostgreSQL RDBMS.
root@test3:~# sudo su - postgres psql
/usr/bin/psql: line 19: use: command not found
/usr/bin/psql: line 20: use: command not found
/usr/bin/psql: line 21: use: command not found
/usr/bin/psql: line 22: use: command not found
...
смонтировал на test3 (VM) диск который был на test2. 
ничего сложного не увидел при настройке, данные таблицы вышли как видим сверху. 
